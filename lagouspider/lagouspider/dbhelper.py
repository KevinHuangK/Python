...
class TestDBHelper():
    ...
    def testCreateTable(self):
        sql = "create table lagou(id int primary key auto_increment,type varchar(100),ename varchar(200),postinfo varchar(200),salary varchar(200),oldurl varchar(200),releasetime varchar(200))"
        self.dbHelper.createTable(sql)

    # 测试插入
    def testInsert(self):
        sql = "insert into lagou(name,url) values(%s,%s)"
        params = ("test", "test")
        self.dbHelper.insert(sql, *params)  # *表示拆分元组，调用insert（*params）会重组成元组

    def testUpdate(self):
        sql = "update lagou set name=%s,url=%s where id=%s"
        params = ("update", "update", "1")
        self.dbHelper.update(sql, *params)

    def testDelete(self):
        sql = "delete from lagou where id=%s"
        params = ("1")
